	...
	<div class="box bg_white pb">
		...
		<c:forEach items="${cart.shopItems}" var="item">
			<tr>
				...
				<td class="img48x20">
					<span class="inb"><img src="${item.sku.product.img.fullPath}"></span>
					<a target="block" href="javascript:void(0);">${fn:substring(item.sku.product.name, 0,30 )}--${item.sku.size}码</a>
				</td>
	 ...
	 <div class="confirm mt">
		...
		<div class="ofc">
			<div class="r">
				<dl class="total">
					<dt>订单金额：<cite>(共<var id="totalNum">${cart.totalProductAmount}</var>个商品)</cite></dt>
					<dd><em class="l">商品金额：</em>￥<var>${cart.totalProductPrice}</var></dd>
					<dd><em class="l">返现：</em>￥<var>0.00</var></dd>
					<dd><em class="l">运费：</em>￥<var>${cart.postage}</var></dd>
					<dd class="orange"><em class="l">应付总额：</em>￥<var id="totalMoney">${cart.totalPrice}</var></dd>
