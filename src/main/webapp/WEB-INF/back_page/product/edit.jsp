			...
			<tbody id="tab_1">
						...
						商品类型:</td><td width="80%" class="pn-fcontent">
								<select name="typeId">
									...
									<c:forEach items="${types }" var="type">
										<option value="${type.id}" <c:if test="${type.id } == ${product.typeId }">selected</c:if>>${type.name}</option>
									</c:forEach>
								</select>
					</td>
				</tr>
					...
					<td width="20%" class="pn-flabel pn-flabel-h">
						...
						<select name="brandId">
							...
							<c:forEach items="${brands }" var="brand">
								<option value="${brand.id }" <c:if test="${brand.id } == ${product.brandId }">selected</c:if>>${brand.name }</option>
							</c:forEach>
						</select>
					</td>
				</tr>
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
						...
						<input type="text" value="0.5" class="required" name="weight" maxlength="10"/>KG
					</td>
				</tr>
				<tr>
					<td width="20%" class="pn-flabel pn-flabel-h">
